...
if (isset($_POST['add_room'])) {
    ...
    $res = insert(
        ...
        [$f['name'], $f['type'], (float)$_POST['price'], (int)$_POST['capacity'], $f['description'], $image],
        "ssdiss"
    );
    $alert_msg = $res
        ? '<div class="alert alert-success alert-dismissible fade show">Thêm phòng thành công! <button class="btn-close" data-bs-dismiss="alert"></button></div>'
...
if (isset($_POST['delete_room'])) {
    ...
    delete("DELETE FROM `rooms` WHERE `id`=?", [$id], "i");
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead class="table-dark">
                        <tr><th>#</th><th>Ảnh</th><th>Tên phòng</th><th>Loại</th><th>Giá/đêm</th><th>Sức chứa</th><th>Trạng thái</th><th>Hành động</th></tr>
                    </thead>
                    <tbody>
                    <?php if ($rooms_res && mysqli_num_rows($rooms_res) > 0):
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                $badge = match($r['status']) { 'booked'=>'badge-booked','maintenance'=>'badge-maintenance',default=>'badge-available' };
                                $label = match($r['status']) { 'booked'=>'Đã đặt','maintenance'=>'Bảo trì',default=>'Trống' };
                                ?>
